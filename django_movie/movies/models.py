...
class Movie(models.Model):
    ...
    year = models.PositiveSmallIntegerField("Дата вывхода", default=2019)
    country = models.CharField("Страна", max_length=30)
    directors = models.ManyToManyField(Actor, verbose_name="режиссер", related_name="film_director")
    actors = models.ManyToManyField(Actor, verbose_name="актеры", related_name="film_actor")
    genres = models.ManyToManyField(Genre, verbose_name="жанры")
    world_premiere = models.DateField("Премьера в мире", default=date.today) # импортировал date
...
class MovieShots(models.Model):
    ...
    movie = models.ForeignKey(Movie, verbose_name="Фильм", on_delete=models.CASCADE) # on delete при удалении фильма все кадры удалятся
...
class Reviews(models.Model):
    ##Отзывы 
    email = models.EmailField() # поле для почты проверяет действительно ли почта 
    name = models.CharField("Имя", max_length=100)
    text = models.TextField("Сообщение", max_length=5000)
    parent = models.ForeignKey(
        'self', verbose_name="Родитель", on_delete=models.SET_NULL, blank=True, null=True
    )
    movie = models.ForeignKey(Movie, verbose_name="фильм", on_delete=models.CASCADE) # отзывы удаляться в месте с фильмом
